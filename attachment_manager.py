...
def fix_bad_attachments_in_file(markdown_file_path, dry_run=False):
    ...
    # Also handles Apple Notes attachments.
    bad_attachment_pattern = r"!?\[.*?\]\([Aa]ttachments\/(.*?)\)"
    replacements = []
    ...
    try:
        with open(markdown_file_path, "r+", encoding="utf-8") as f:
            ...
            if replacements:
                if dry_run:
                    print(
                        f"[DRY RUN] In {markdown_file_path}, would fix {len(replacements)} bad links:"
                    )
                    ...
                    f.truncate()
                    print(
                        f"Fixed {len(replacements)} bad attachment links in {markdown_file_path}"
                    )
        ...
        with open(markdown_file_path, "r+", encoding="utf-8") as f:
            ...
            for match in re.finditer(attachment_pattern, content):
                ...
                if os.path.exists(old_attachment_path):
                    ...
                    if dry_run:
                        print(
                            f"[DRY RUN] Would rename: {old_attachment_path} -> {new_attachment_path}"
                        )
                    else:
                        os.rename(old_attachment_path, new_attachment_path)

                    new_content = new_content.replace(
                        original_link, f"![[{new_file_name}]]\n\n"
                    )
...
def rename_attachments_in_canvas_file(canvas_file_path, attachments_dir, dry_run=False):
    """Renames attachments found in a single canvas file."""
    attachment_pattern = r'"file":"attachments\/([^/"]+)"'
    markdown_file_name = os.path.splitext(os.path.basename(canvas_file_path))[0]
    ...
    try:
        with open(canvas_file_path, "r+", encoding="utf-8") as f:
            ...
            if changes_made:
                if dry_run:
                    print(f"[DRY RUN] Would update links in: {canvas_file_path}")
                else:
                    if new_content != original_content:
                        f.seek(0)
                        f.write(new_content)
                        f.truncate()
                        print(f"Updated attachments in {canvas_file_path}")
            else:
                print(f"No changes made to {canvas_file_path}")

    except Exception as e:
        print(f"Error processing {canvas_file_path}: {e}")


def main():
    print("Starting attachment manager...")
    parser = argparse.ArgumentParser(
        description="Rename attachments in markdown files."
    )
    parser.add_argument(
        "--markdown-dir", required=True, help="Directory containing markdown files."
    )
    parser.add_argument(
        "--attachments-dir",
        required=True,
        help="Directory containing attachment files.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
